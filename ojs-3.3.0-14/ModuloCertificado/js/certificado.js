...
const getConsulta = async ()=>{
    try{
        ...
        res = await fetch("http://localhost/ojs-3.3.0-14/ojs-3.3.0-14/ModuloCertificado/Handler/certificado_handler.php", options),
        json = await res.json();
        ...
        if(json.length !== 0) {
            ...
            json.forEach(el => {
                ...
                $template.querySelector(".btn .btn_certificates").href=`http://localhost/ojs-3.3.0-14/ojs-3.3.0-14/ModuloCertificado//pages/certificado.inc.php?idSub=${el.SUBMISSION_ID}&journal=${el.JOURNAL}`;
